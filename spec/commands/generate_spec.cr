...
describe Roadshow::Commands::Generate do
  ...
  it "generates Docker and Docker Compose configuration with multiple services" do
    SpecHelper.in_project("databases") do
      begin
        FileUtils.mv("scenarios", "expected_scenarios")
        FileUtils.mkdir("scenarios")
        FileUtils.cp("expected_scenarios/postgres.gemfile", "scenarios")
        FileUtils.cp("expected_scenarios/mysql.gemfile", "scenarios")
